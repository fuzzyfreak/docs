...
fdelete_nas()
{
    ...
    for i in $G24FILE; do echo "Deleting files 48 hours or older on nas $i"; rm $i; done
        if [ "$?" -eq "0" ]; then
...
fdelete()
{
    ...
    for i in $G12FILE; do echo "Deleting files 12 hours or older on fdns $i"; rm $i; done
        if [ "$?" -eq "0" ]; then
                echo "Delete successfull" >> $TSOURCE/$FL
                say_this "say Delete successfull!"
        else    
